...
\begin{itemize}
    ...
    a secure and controlled local infrastructure — a requirement the current system 
    cannot fulfill.
\end{itemize}

These two limitations — insufficient Tunisian legal knowledge and reliance on external 
...
The complete system — language model, \ac{RAG} pipeline, and vector database — will 
...
\begin{itemize}
    ...
    \item \textbf{Communication Phase:} \ac{DSR} explicitly includes a communication 
    activity that maps directly onto the writing of this report and its oral defense, 
    making the methodology fully consistent with the academic context of this work.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figures/azure_ai_foundry.jpg}
...
Azure AI Foundry was chosen for its enterprise-grade security guarantees, its native 
...
We chose VS Code mainly for practical reasons: it runs smoothly on modest 
...
Python was chosen for its central role in the \ac{AI} ecosystem and the 
...
n8n was selected mainly because it is open-source and self-hostable, which 
...
FastAPI was chosen because it makes exposing Python functions as web endpoints 
...
This chapter established the general framework of our graduation project. We began 
by presenting E-Tafakna, a pioneering Tunisian LegalTech startup operating across 
the \ac{MENA} region, whose platform digitizes the entire legal document lifecycle 
and integrates two \ac{AI}-powered agents — Elyssa and Kahina — to assist users 
with document drafting, compliance, and legal guidance.

A critical analysis of the existing system revealed two fundamental limitations: 
the reliance on general-purpose language models that lack grounding in Tunisian 
legal knowledge, and a dependency on external cloud services that is incompatible 
with the on-premises deployment requirements of \ac{ATI} Tunisie. These limitations 
gave rise to a central research question — how to build a legal \ac{AI} assistant 
that is both deeply specialized in Tunisian law and fully deployable within a secure, 
self-contained local infrastructure.

In response, we proposed a solution built around three complementary components: 
a fine-tuned \ac{SLM} or \ac{LLM} trained on Tunisian legal corpora, a \ac{RAG} 
pipeline for dynamic legal knowledge retrieval, and a fully on-premises deployment 
at \ac{ATI} Tunisie using a local inference framework.

Finally, after evaluating several methodological frameworks — from traditional 
project management approaches such as Scrum and Kanban, to data science methodologies 
such as \ac{CRISP-DM} and \ac{TDSP}, and research-oriented paradigms — we adopted 
\ac{DSR} as our guiding methodology. Its artifact-centered structure, formal 
evaluation framework, and explicit communication phase make it the most appropriate 
choice for a project that is both technically ambitious and academically rigorous.

The following chapter will build on this foundation by providing a detailed review 
of the state of the art, examining the key concepts, existing approaches, and 
technologies that underpin our proposed solution.