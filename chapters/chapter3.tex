...
\begin{sidewaysfigure}
\centering
\includegraphics[width=1.0\textheight,height=0.6\textwidth]{figures/flowchart_pipeline.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figures/n8n_workflow_simple.png}
...
Both models were trained on OVHcloud \cite{ovhcloud2024ainotebooks} AI Notebook instances, as shown in Figure~\ref{fig:ovh-notebook}. Gemma~4~E4B was trained on a single NVIDIA Tesla V100S with 32 GiB of \ac{VRAM} using standard \ac{LoRA} with an fp16 base. Qwen3.5~9B was trained on a single NVIDIA Tesla V100S using \ac{QLoRA}, with the base model loaded in 4-bit NF4 quantization. The quantitative results for both runs, including loss curves, training duration, and peak \ac{VRAM} usage, are reported in Chapter~IV alongside the evaluation experiments.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/ovh_notebook.png}
\caption{OVHcloud AI Notebook Instance Used for Fine-Tuning}
\label{fig:ovh-notebook}
\end{figure}
