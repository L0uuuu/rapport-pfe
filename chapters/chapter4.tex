...
\label{subsec:ch4-streamlit}

To make the \ac{RAG} pipeline accessible beyond the terminal, a lightweight web interface was developed using Streamlit~\cite{streamlit2024docs} and is located at \texttt{rag/streamlit\_app.py}. The interface wraps the same \texttt{rag/query.py} pipeline without modifying any of its logic: the embedding model, the Qdrant retrieval step, the cross-encoder reranker, the confidence gate, and the Ollama generation call all execute identically to the command-line version.
...
Figure~\ref{fig:streamlit-interface} shows the interface during a live query session.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/streamlit_interface.png}
\caption{Streamlit Web Interface for the RAG Query Pipeline}
